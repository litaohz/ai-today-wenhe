...
INPUT_MD = os.path.join(BASE_DIR, "archives-260128", "ai_posts_summary_2026-01-28.md")
OUTPUT_HTML = os.path.join(BASE_DIR, "archives-260128", "ai_posts_summary_2026-01-28.html")
...
STYLE_CSS_DEST = os.path.join(BASE_DIR, "archives-260128", "style.css")
...
def process_time_string(time_str):
    ...
    # This handles "2026-01-27 17:59 UTC" 
    # AND "2026-01-27 23:51 UTC (北京时间...)" because re.search finds the first match
    ...
    # Fallback if no UTC pattern found
    return time_str
...
def parse_markdown(md_content):
    ...
    for i in range(1, len(sections)):
        ...
        # Sometimes markdown has "Entity ** Title" or "Entity - Title"
        # Assuming standard format "Entity - Title"
        if ' - ' in full_title:
        ...
        # Clean brackets from title if present: "[Title]" -> "Title"
        # This handles cases where brackets are in the title string
...
def generate_html(data, template_path, output_path):
    ...
    for item in data.get('items', []):
        ...
        # Process and convert time
        raw_time = item.get('time', '')
        time_str = process_time_string(raw_time)
        time_str = clean_punctuation(time_str) # Just in case
        ...
        for group in item.get('content_groups', []):
            ...
            for list_item in group['items']:
                ...
                # Remove brackets [Text] -> Text if inside bold
                def clean_bold_brackets(match):
                    text = match.group(1)
                    if text.startswith('[') and text.endswith(']'):
                       text = text[1:-1]
                    return f'<strong>{text}</strong>'

                li_text = re.sub(r'\*\*(.*?)\*\*', clean_bold_brackets, li_text)
...
def main():
    ...
    if not os.path.exists(STYLE_CSS_DEST):
        ...
        # Ensure we always update CSS if source changed
        shutil.copy(STYLE_CSS_SRC, STYLE_CSS_DEST)
        print(f"Updated CSS at {STYLE_CSS_DEST}")
