...
HTML生成时，所有时间**必须**统一转换为北京时间显示：

1. **识别 UTC 时间**：如果 Markdown 中的时间是 UTC 格式（如 `2026-01-27 17:59 UTC`），必须将其转换为 UTC+8 的北京时间。
2. **只保留北京时间**：输出时只显示转换后的北京时间，格式为 `YYYY-MM-DD HH:MM 北京时间`。不要同时显示 UTC 时间。
3. **示例**：
   - Markdown: `2026-01-27 17:59 UTC` → HTML: `2026-01-28 01:59 北京时间`
